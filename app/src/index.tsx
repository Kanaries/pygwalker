...
const MainApp = observer(
    (props: { children: React.ReactNode; darkMode: "dark" | "light" | "media"; hideToolBar?: boolean; gid?: string; sendMessage?: boolean }) => {
        ...
        const sendAppearanceMessageToParent = useCallback(
...
const ExploreApp: React.FC<IAppProps & { initChartFlag: boolean }> = (props) => {
    ...
    const disposerRef = React.useRef<() => void>(null);
    ...
    return (
        <React.StrictMode>
            ...
            {mode === "walker" ? (
                ...
                <GraphicRendererApp {...props} dataSource={props.dataSource} visSpec={visSpec} />
            )}
            <Options {...props} />
        </React.StrictMode>
    );
    ...
    return (
        <React.StrictMode>
            <div className="flex">
                ...
                {commonStore.isStreamlitComponent && expand && (
                    <ChevronLeftIcon className="h-6 w-6 cursor-pointer border border-black-600 rounded-full" onClick={() => setExpand(false)}></ChevronLeftIcon>
                )}
                {commonStore.isStreamlitComponent && !expand && (
                    <ChevronRightIcon
                        className="h-6 w-6 cursor-pointer border border-black-600 rounded-full"
