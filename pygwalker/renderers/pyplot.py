...
def build_code(payload: Dict[str, any], size: Dict[str, int], df_name = 'df'):
    ...
    def plot_single(plot_func, plot_options = ''):
        ...
        if fid_text:
            code_storage.add_code(f"plt.text({df_name}['{fid_x}'], {df_name}['{fid_y}'], {df_name}['{fid_text}'])")
    ...
    def plot_box_single():
        ...
        if fid_y is None and fid_x is not None:
            code_storage.add_code(f"plt.boxplot({df_name}['{fid_x}'], patch_artist=True, vert=False, labels=['{name_x}'])")
            
        elif fid_x is None and fid_y is not None:
            code_storage.add_code(f"plt.boxplot({df_name}['{fid_y}'], patch_artist=True, labels=['{name_y}'])")
        else:
    ...
    def plot_arc_single_only():
        ...
        label_string = f"labels={df_name}['{fid_color}'], " if fid_color else ''
        code_storage.add_code(f"plt.pie({df_name}['{fid_theta}'], autopct='%1.1f%%', {label_string}{color_string})")
        if colorbar_string:
            code_storage.add_code(colorbar_string)
