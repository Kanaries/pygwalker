...
def get_dimension_count_string(df_name: str, id: str,fields: List[Dict[str, any]], is_agg: bool):
    ...
    if len(facet_fields) > 0:
        dim_fields = f"{id} = list(itertools.product(" + ",".join([f"{df_name}['{get_fid_with_agg(field, is_agg)}'].unique()" for field in facet_fields]) + "))"
...
def build_code(payload: Dict[str, any], size: Dict[str, int], df_name = 'df', numpy_name = 'np', instance_name='plt'):
    ...
    if fid_color is None:
        ...
        if color_field.get("semanticType") == "quantitative" or color_field.get("semanticType") == "temporal":
            ...
            if mark != 'rect':
                code_storage.add_code(f"unique_color = {df_name}['{fid_color}'].unique()")
                code_storage.add_code(f"color_map = dict(zip(unique_color, itertools.cycle(cp)))")
            colorbar_string = f"{instance_name}.legend([mpatches.Patch(color=color, label=label) for (label, color) in color_map.items()], unique_color, loc=1, title='{get_name_with_agg(color_channel[0], is_aggergated)}')"
            color_type = 'category'
    def get_color_string(df_name):
        if fid_color:
            if color_type == 'category':
                return f"c={df_name}['{fid_color}'].map(color_map),"
            else:
                return f"c={df_name}['{fid_color}'],cmap=color_map,"
        return f"color='{primary_color}', "
    def get_opacity_string(df_name, ax_name):
        if fid_opacity:
            ...
            if semanticType == 'quantitative' or semanticType == 'temporal':
                code_storage.add_code(f"alpha = {df_name}['{fid_opacity}']")
                code_storage.add_code(f"alpha = (alpha - alpha.min()) / (alpha.max() - alpha.min()) * 0.7 + 0.3")
                opacity_bar_string = f"{ax_name}.figure.colorbar(ScalarMappable(norm=Normalize(vmin={df_name}['{fid_opacity}'].min(), vmax={df_name}['{fid_opacity}'].max()), cmap='Greys'), ax={ax_name}, label='{get_name_with_agg(opacity_field, is_aggergated)}')"
            else:
                code_storage.add_code(f"alpha = {df_name}['{fid_opacity}']")
                ...
                code_storage.add_code("alpha = alpha.map(alpha_map)")
                opacity_bar_string = f"opacity_legend = {ax_name}.legend([mpatches.Patch(color='white', label=label, alpha=alpha) for (label, alpha) in alpha_map.items() if isinstance(label, str)], unique_alpha, loc=4, title='{get_name_with_agg(opacity_field, is_aggergated)}')\n{ax_name}.add_artist(opacity_legend)"
            return (opacity_string, opacity_bar_string)
        return ('', '')
    ...
    def plot_square_single(ax_name, fig_name, df_name, x_field, y_field):
        ...
        x_string = f"columns='{fid_x}', " if fid_x else ''
        y_string = f"index='{fid_y}', " if fid_y else ''
        color_string = ''
        colorbar_string = ''
        opacity_bar_string = ''
        imdata = "data"
        ...
        if fid_color:
            if color_type == 'category':
                code_storage.add_code(f"data = {df_name}.pivot_table({y_string}{x_string}values='{fid_color}', aggfunc=lambda x: ','.join(sorted(set(x))))")
                ...
                code_storage.add_code(f"imdata = np.array(list(data.replace(color_map).map({instance_name}.matplotlib.colors.to_rgb).map(list).values.flatten())).reshape(data.shape + (3,))")
                colorbar_string = f"{ax_name}.legend([mpatches.Patch(color=color, label=label) for (label, color) in color_map.items() if isinstance(label, str)], unique_color, loc=1, title='{get_name_with_agg(color_channel[0], is_aggergated)}')"
                imdata = "imdata"
            else:
                code_storage.add_code(f"data = {df_name}.pivot_table({y_string}{x_string}values='{fid_color}')")
                color_string = f"cmap=color_map, "
                colorbar_string = f"{ax_name}.figure.colorbar(im, label='{get_name_with_agg(color_channel[0], is_aggergated)}')"
        else:
            group_y_string = f"'{fid_y}'," if fid_y else ''
            group_x_string = f"'{fid_x}'," if fid_x else ''
        ...
        if fid_opacity:
            ...
            opacity_string = f"alpha=alpha, "
            if semanticType == 'quantitative' or semanticType == 'temporal':
                code_storage.add_code(f"alpha = {df_name}.pivot_table({y_string}{x_string}values='{fid_opacity}', fill_value=0)")
                code_storage.add_code(f"alpha = (alpha - {df_name}['{fid_opacity}'].min()) / ({df_name}['{fid_opacity}'].max() - {df_name}['{fid_opacity}'].min()) * 0.7 + 0.3")
                opacity_bar_string = f"{ax_name}.figure.colorbar(ScalarMappable(norm=Normalize(vmin={df_name}['{fid_opacity}'].min(), vmax={df_name}['{fid_opacity}'].max()), cmap='Greys'), ax={instance_name}, label='{get_name_with_agg(opacity_field, is_aggergated)}')"
            else:
                code_storage.add_code(f"alpha = {df_name}.pivot_table({y_string}{x_string}values='{fid_opacity}', aggfunc=lambda x: ','.join(sorted(set(x))))")
        ...
        if fid_y:
            ...
            code_storage.add_code(f"{ax_name}.set_ylabel('{get_name_with_agg(y_field, is_aggergated)}')")
        if fid_x:
            ...
            code_storage.add_code(f"{ax_name}.set_xlabel('{get_name_with_agg(x_field, is_aggergated)}')")
    ...
    def plot_scatter_single(ax_name, fig_name, df_name, x_field, y_field, marker):
        ...
        if fid_shape:
            ...
            code_storage.add_code(f"shape_map = dict(zip({df_name}['{fid_shape}'].unique(), itertools.cycle(shapes)))")
            code_storage.add_code(f"for (shape, marker) in shape_map.items():")
            code_storage.set_indent_level(1)
            code_storage.add_code(f"sub_data = {df_name}[{df_name}['{fid_shape}'] == shape]")
            color_string = get_color_string('sub_data')
            size_string = f"s=sub_data['{fid_size}'], " if fid_size else ''
            opacity_string, opacity_bar_string = get_opacity_string('sub_data', ax_name)
            code_storage.add_code(f"{ax_name}.scatter(sub_data['{fid_x}'], sub_data['{fid_y}'], {size_string}marker=marker, {color_string}{opacity_string})")
            code_storage.set_indent_level(0)
            
            code_storage.add_code(f"shape_legend = {ax_name}.legend(handles=[{instance_name}.Line2D([0], [0], marker=marker, label=shape) for shape, marker in shape_map.items()],loc=7, title='{get_name_with_agg(shape_channel[0], is_aggergated)}')")
            ...
            size_string = f"s={df_name}['{fid_size}'], " if fid_size else ''
            opacity_string, opacity_bar_string = get_opacity_string('sub_data', ax_name)
    ...
    def plot_bar_single(ax_name, fig_name, df_name, x_field, y_field):
        ...
        color_string = get_color_string(df_name)
        code_storage.add_code(f"{ax_name}.bar({x_string}{y_string}{change_color_string_key(color_string, 'color')}{opacity_string}{size_string})")
        if colorbar_string:
            code_storage.add_code(colorbar_string)
        if fid_x:
            code_storage.add_code(f"{ax_name}.set_xlabel('{get_name_with_agg(x_field, is_aggergated)}')")
    ...
    def plot_single(ax_name, fig_name, df_name, plot_func, x_field, y_field):
        fid_x = get_fid_with_agg(x_field, is_aggergated)
        fid_y = get_fid_with_agg(y_field, is_aggergated)
        x_string = f"{df_name}['{fid_x}'], " if fid_x else ''
        y_string = f"{df_name}['{fid_y}'], " if fid_y else ''
        ...
        code_storage.add_code(f"{ax_name}.{plot_func}({x_string}{y_string}{color_string}{opacity_string}{size_string})")
        if colorbar_string:
            code_storage.add_code(colorbar_string)
        if fid_x:
            code_storage.add_code(f"{ax_name}.set_xlabel('{get_name_with_agg(x_field, is_aggergated)}')")
    ...
    def plot_box_single(ax_name, fig_name, df_name, x_field, y_field):
        ...
        if fid_y is None and fid_x is not None:
            code_storage.add_code(f"{ax_name}.boxplot({df_name}['{fid_x}'], patch_artist=True, vert=False, labels=['{name_x}'])")
            
        elif fid_x is None and fid_y is not None:
            code_storage.add_code(f"{ax_name}.boxplot({df_name}['{fid_y}'], patch_artist=True, labels=['{name_y}'])")
        else:
    ...
    def plot(ax_name, fig_name, df_name, x_field, y_field):
        ...
        elif mark in plot_types:
            plot_single(ax_name, fig_name, df_name, plot_types[mark], x_field, y_field)
