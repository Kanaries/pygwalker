...
def get_fid_with_agg(field, is_aggergated):
    if field is None:
        return None
    fid = field.get("fid")
    if fid == MEA_VAL_FID:
    ...
    if is_aggergated and aggName and analytic_type == 'measure':
        return f"{fid}_{aggName}"
    return fid
    
def get_name_with_agg(field, is_aggergated):
    if field is None:
        return None
    name = field.get("name")
    analytic_type = field.get("analyticType")
...
def to_colors(flat_palette):
    # split every 6 chars to a list
    return ['#' +flat_palette[i:i + 6] for i in range(0, len(flat_palette), 6)]

def get_color_palette(name: str):
    if name in palettes:
        return to_colors(palettes[name])
    return None

def get_spec_color_palette(spec):
    layout = spec.get("layout")
    if layout is None:
        return None
    return get_color_palette(layout.get("colorPalette"))

def get_primary_color(spec):
    layout = spec.get("layout")
    if layout is None:
        return None
    return layout.get("primaryColor")
