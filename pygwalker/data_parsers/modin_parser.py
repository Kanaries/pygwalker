...
class ModinPandasDataFramePropParser(DataFramePropParser[mpd.DataFrame]):
    ...
    @classmethod
    def _infer_semantic(cls, s: mpd.Series):
    ...
    @classmethod
    def _infer_analytic(cls, s: mpd.Series):
        kind = s.dtype.kind
        return 'measure' if \
    ...
    @classmethod
    def _series(cls, df: mpd.DataFrame, i: int, col: str):
        return df.iloc[:, i]

    @classmethod
    def _to_matrix(cls, df: mpd.DataFrame) -> List[List[Any]]:
        df = df.replace({float('nan'): None})
        return df.to_dict(orient='tight')

    @classmethod
    def _decode_fname(cls, s: mpd.Series):
        fname = fname_decode(s.name)
        fname = json.dumps(fname, ensure_ascii=False)[1:-1]
        return fname

    @classmethod
    def escape_fname(cls, df: mpd.DataFrame):
        df = df.reset_index()
